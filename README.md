...
GitHub Actions で push / pull_request 時に以下 7 ジョブを並列実行している（[`.github/workflows/ci.yml`](.github/workflows/ci.yml)）。

- **Backend**: Ruff (lint) / Pytest + coverage / 検索精度評価 (simulate)
...
- Pre-commit hook（Ruff / gitleaks）で commit 時に軽いチェック（[`.pre-commit-config.yaml`](.pre-commit-config.yaml)）
...
cp .env.example .env          # AI モードを使う場合は OPENAI_API_KEY を設定
...
```bash
docker-compose exec rag-api bash -lc "\
  python /app/tools/index-builder/preprocess.py --input /app/data/demo/demo_logs.csv --output /app/data/processed && \
  python /app/tools/index-builder/build_index.py --input /app/data/processed/chunks.jsonl --output /app/data/indices"
```
...
AI モードの利用には `OPENAI_API_KEY` が必要。未設定でも検索機能は動作する。
