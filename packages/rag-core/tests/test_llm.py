"""
LLMService のロジック単体テスト

OpenAI API は呼ばず、事前チェック・信頼度判定・スコア計算など
純粋なロジックだけを検証する。
"""
...
class TestCalculateDisplayScore:
    ...
    def test_out_of_range_positive(self):
        # 3.0 は Cross-Encoder の logit の典型値、[0, 1] に正規化される
...
class TestExtractSources:
    ...
    def test_score_is_normalized(self, llm):
        ...
        sources = llm.extract_sources(results)
        # rerank_score が優先され [0, 1] に正規化される
