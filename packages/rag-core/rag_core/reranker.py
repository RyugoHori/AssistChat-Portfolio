...
Cross-Encoderで検索結果を再順位付け。
Bi-Encoderより精度は高いが処理速度が遅いため、
上位候補のみに適用して速度と精度のバランスを最適化。
"""
...
class Reranker:
    ...
    Bi-Encoderは高速だが、クエリと文書を独立にエンコードするため
    詳細なニュアンスの把握には限界がある。
    Cross-Encoderはペアで処理するため精度が向上するが、
    処理コストが高いため上位候補のみに適用。
    """
    ...
    def rerank(
        ...
        元のスコアはoriginal_scoreに保存し、
        新しいスコアはrerank_scoreに格納。
        """
