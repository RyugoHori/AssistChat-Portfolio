...
class Chunk:
    ...
    テキストをEmbeddingに適したサイズに分割した単位。
    
    実データ対応:
    実データは50字〜2000字と文章長がバラバラなため、
    以下の戦略でチャンク分割を実施:
    
    - 短文（200字未満）: チャンク分割しない（1チャンク = 1文書）
    - 中文（200-800字）: 500字で分割、100字オーバーラップ
    - 長文（800字以上）: 500字で分割、150字オーバーラップ
    
    長文ほどオーバーラップを大きくすることで、
    文脈の切れ目での情報損失を最小化。
    """
    chunk_id: str
    text: str
...
class TextChunker:
    ...
    デフォルト設定: chunk_size=500、overlap=50
    保全記録は1件あたり200〜500文字程度のため、
    多くの場合1文書が1チャンクとなる。
    """
    ...
    def chunk_text(
        ...
        実装の考慮点:
        - 実データは文章長が50〜2000字と大きくバラつく
        - 短文（<200字）はそのまま1チャンクとして扱う
        - 長文は文脈を保持するためオーバーラップを設ける
        - 設備保全記録は「原因→対処→結果」の流れがあるため、
          この流れが分断されないよう500字単位で分割
        """
        if not text:
            return []
        ...
        for sentence in sentences:
            ...
            if current_chunk and len(potential_chunk) > self.chunk_size:
                if current_chunk:
                    ...
                    # オーバーラップ処理（前チャンクの末尾を次チャンクの先頭に含める）
